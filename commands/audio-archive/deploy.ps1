# Audio Archive Cloud Function Deployment Script (PowerShell)
...
Write-Host "=== Audio Archive Deployment ===" -ForegroundColor Cyan
...
gcloud functions deploy $FUNCTION_NAME `
    ...
    --set-env-vars="GCS_BUCKET=$BucketName,LOCAL_TIMEZONE=Pacific/Auckland" `
    --memory=1024MB `
    --timeout=540s `
    --max-instances=5
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "The audio-archive function is now listening for transcript events."
    Write-Host "When otter-sync uploads a new transcript, this function will:"
    Write-Host "  1. Download the transcript from GCS"
    Write-Host "  2. Extract the audio URL"
    Write-Host "  3. Download the audio file"
    Write-Host "  4. Save to gs://$BucketName/audio/"
    Write-Host "  5. Update transcript with audio path reference"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  # List archived audio files"
    Write-Host "  gsutil ls gs://$BucketName/audio/"
    Write-Host ""
    Write-Host "  # List audio files with sizes"
    Write-Host "  gsutil ls -l gs://$BucketName/audio/"
    Write-Host ""
    Write-Host "  # Check state file (processed transcripts)"
    Write-Host "  gsutil cat gs://$BucketName/.audio_archive_state.json"
    Write-Host ""
    Write-Host "  # Clear state to force re-archiving (use with republish-events)"
    Write-Host "  gsutil rm gs://$BucketName/.audio_archive_state.json"
    Write-Host ""
    Write-Host "  # Download an audio file"
    Write-Host "  gsutil cp gs://$BucketName/audio/FILENAME.mp3 ./local_file.mp3"
} else {
    Write-Host ""
